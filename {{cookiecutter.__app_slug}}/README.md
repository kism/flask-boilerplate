...
poetry install
poetry shell
flask --app {{cookiecutter.__app_slug}} run --port 5000
```
...
.venv/bin/waitress-serve \
    ...
    --threads 4 \
    --call {{cookiecutter.__app_slug}}:create_app
```
...
To get code coverage locally, the config is set in 'pyproject.toml', or run with `pytest --cov={{cookiecutter.__app_slug}} --cov-report=term --cov-report=html`
